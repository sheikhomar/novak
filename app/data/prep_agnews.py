...
class TransformerFeaturizer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        print(f"""Transforming {len(X)} documents to using '{self._model_name}'...""")
...
class AverageWordVectorFeaturizer(BaseEstimator, TransformerMixin):
    def __init__(
        self, dim: int = 100, vector_model_name: str = "glove-wiki-gigaword-100"
    ):
        self._dim = dim
...
def get_data_splits_from_zip_file(zip_path: Path) -> Dict[str, object]:
    data_splits = {}
...
class DataPreparation:
    ...
    def _process_features_and_labels(
        ...
        for split_name, docs in docs_map.items():
            ...
            split_path = (
                self._output_dir
                / "processed"
                / f"agnews-{split_name}-{featurizer_name}.npz"
            )
        ...
        for split_name in data_splits:
            ...
            for item in data_splits[split_name]:
                ...
                if len(desc) > 0:
                    ...
                    print("Discarding item because description is empty")
                    print(item)
            print(
                f"Read {len(docs[split_name])} documents from the {split_name} split."
            )
        return docs, labels
    ...
    def _download_file(self, url: str, output_path: Path) -> None:
        ...
        with requests.get(url, stream=True) as r, open(output_path, "wb") as f:
            ...
            with tqdm(total=total_length, unit="iB", unit_scale=True) as pbar:
